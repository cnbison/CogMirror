...
- **工程线**：Phase 0 静态题库最小链路可运行——做题（静态题库 + 确定性 partial credit 判分）→ 5D 信念更新（K/P/S/C/X）→ 命令行认知地图（5 维状态 / Bloom 六层 / 伪自信 / 临界概念 / 一句话建议）。认知地图三大核心元素（伪自信 / 临界概念 / 一句话建议）已全部做成"做题当下的实时信号"。题库 51 题（5 个 topic × Bloom L1-L6 全覆盖，2026-08-27 补齐 L5/L6），**294 项自动化测试通过**。近期功能：29 道选择题逐选项讲解、错题重练 `--review`、认知地图「与上次相比」对比段、返回用户进度概览、**P1 黄金回归基建**（7 条合成学习者序列 + 数值基线快照，引擎行为变更自动判回归，2026-08-28，见 [docs/implementation-plan.md](./docs/implementation-plan.md) 第 2 节）、**P2 自评校准曲线**（伪自信折扣从固定 0.15 升级为按自评分桶的历史真实答对率驱动 + 地图「自评校准度（ECE）」行，2026-08-31，见方案第 3 节）。自测记录见 [SELF-TEST-CHECKLIST.md](./SELF-TEST-CHECKLIST.md)
