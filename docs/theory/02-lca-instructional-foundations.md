...
> ⚠️ **⚠⚠ CogMirror 迁移标注（2026-08-22，必读）⚠⚠**
>
> 本文档自 ECOS 原样迁移，作教学法理论参考，用于设计"一句话建议"的呈现
> 逻辑（例如间隔效应可以决定什么时候提醒复习）。**注意**：文中与双 Agent
> 互校、K12 场景相关的表述是 ECOS 历史语境，CogMirror 不继承（双 Agent
> 互校的 H3 假设经验证未成立，见 LESSONS-FROM-ECOS.md 教训1）。本文的
> 认知负荷理论、Bjork 合意困难、认知学徒制部分与具体场景无关，可继续参考。
