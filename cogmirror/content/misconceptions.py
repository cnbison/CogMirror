"""Python 基础 Misconception Library.

直接迁移自 ECOS `ecos/cta/content/python_basics_misconceptions.py`
（MIGRATION.md 第1类，内容未改，import 解耦）。
...
class PythonBasicsMisconceptionLibrary:
    ...
    _entries: ClassVar[list[MisconceptionEntry]] = [
        ...
        MisconceptionEntry(
            ...
            correction_strategy="scope_namespace_explanation",
            bloom_layer=BloomLevel.APPLY,
        ),
        # ─── 循环 ─────────────────────────────────────────────────
        MisconceptionEntry(
    ...
    def detect_by_keywords(self, text: str) -> MisconceptionEntry | None:
        """关键词匹配兜底检测（迁移自 ECOS 的非 LLM 路径思路）.

        命中多个时取关键词命中数最多的条目；无命中返回 None。
        """
        best: MisconceptionEntry | None = None
        best_count = 0
        for entry in self._entries:
            count = sum(1 for kw in entry.detection_keywords if kw in text)
            if count > best_count:
                best = entry
                best_count = count
        return best
