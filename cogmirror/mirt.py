...
直接迁移自 ECOS `ecos/cta/l2_mirt.py`（逻辑未改）。
MAP（最大后验）估计，Bi-factor 简化结构：
  θ ∈ ℝ⁵ -- 5 个特化维度 (K/P/S/C/X)
  G ∈ ℝ  -- 一般因子（用 mean(θ) 近似）

局限说明：MIRT 框架依赖结构化的题目-参数矩阵，只对"做题"场景成立
（见 PRD 第8节与 docs/theory/ 下的局限标注）。
"""
...
class BiFactorMIRT5D:
    """Bi-factor 5D MIRT（简化版）--单用户 MAP 估计."""
    ...
    def predict_probability(theta: np.ndarray, item: MIRTItemParams) -> float:
        """预测用户答对该题的概率 P(correct | θ, item)."""
        g = float(np.mean(theta))  # 一般因子（简化）
